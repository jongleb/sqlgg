...
let create_table_migration ct =
  let create = create_line ct in
  match List.map index_add_action ct.indexes with
  | [] -> [create]
  | actions ->
    alter_table_sql ~default_sql_lookup:no_default ct.table (Columns actions)
    |> Option.map_default (fun s -> [create; s]) [create]

let create_table_schema ct =
  let clauses =
    Option.map_default (fun c -> [Default_charset c]) [] ct.charset
    @ List.map (fun ix -> Add_key ix) ct.indexes
  in
  create_line ~extra:(List.filter_map inline_index_clause ct.indexes) ct
