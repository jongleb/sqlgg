...
module Join_ctors = struct
  ...
  let of_vars vars =
    ...
    in
    List.fold_left2 (fun acc (join_id, _) ctor -> M.add join_id ctor acc) M.empty joins ctors

  let ctor t join_id =
    match M.find_opt join_id t with
    | Some c -> c
    | None -> fail "unknown dynamic join %d" join_id

  let sources t = M.bindings t |> List.map snd |> List.sort_uniq compare
  ...
  let rec loop app = function
    ...
    | DynamicSelectJoin { pid; join_id; join_text } :: tl ->
      ...
      bprintf b {|(if List.mem %s %s.deps then " " ^ %s else "")|}
        (Join_ctors.ctor join_ctors join_id) (make_param_name 0 pid) (quote join_text);
      loop true tl
...
let emit_dynamic_select_body ~outside ~module_kind ~dynamic_infos stmt =
  ...
  let join_ctors = Join_ctors.of_vars stmt.Gen.vars in

  let col_ref di =
    if Join_ctors.is_empty join_ctors then di.param_name else di.param_name ^ ".projection"
  in
    ...
    | Gen.DynamicSelectJoin { pid; join_id; join_text } :: rest ->
      ...
      let expr =
        sprintf {|(if List.mem %s %s then " " ^ %s else "")|}
          (ctor_qual di (Join_ctors.ctor join_ctors join_id)) (deps_ref di) (quote join_text)
      in
...
let generate_stmt ~module_kind index stmt =
  ...
  let sql = make_sql ~join_ctors:(Join_ctors.of_vars stmt.Gen.vars) @@ get_sql stmt in
...
let generate_dynamic_select_modules stmts =
  List.iteri (fun index stmt ->
    ...
    let sql_pieces = get_sql stmt in
    let join_ctors = Join_ctors.of_vars stmt.Gen.vars in
    let source_ctors = Join_ctors.sources join_ctors in
    let deps_of_field (field : _ Sql.dynamic_field) =
      match field.Sql.join_deps with
      | [] -> "[]"
      | ids -> sprintf "[%s]" (ids |> List.map (Join_ctors.ctor join_ctors) |> String.concat "; ")
    in
