    ...
    assert_equal 
      ...
      (List.map 
        (
          ...
          | DynamicSelectJoin _ -> assert false
          ) 
        stmt.Gen.vars)
...
let do_test ?kind sql schema params =
  ...
  (List.map (function Single (p, _) -> p | SharedVarsGroup _ | OptionActionChoice _ | SingleIn _ | Choice _ | ChoiceIn _ | TupleList _ -> assert false | DynamicSelect _ -> failwith "dynamic selects not supported for this host language" | DynamicSelectJoin _ -> assert false) stmt.vars);
