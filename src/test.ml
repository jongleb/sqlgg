...
let cmp_attrs (a : Sql.attr list) b =
  Int.equal (List.compare_lengths a b) 0
  && List.for_all2 (fun x y ->
      String.equal x.name y.name && Type.equal x.domain y.domain
      && Constraints.equal x.extra y.extra && Meta.equal x.meta y.meta) a b
...
let test_type_mapping_params _ = 
  ...
  assert_equal 
    ...
    [
      ...
      attr' ~extra:[] "d" (Decimal { precision = Some 10; scale = Some 2; });
      attr' "e" Int;
    ] 
    (schema_to_attrs stmt.schema);
