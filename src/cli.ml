...
let parse_args () =
  ...
  [
    ...
    "-migrations-file", Arg.String (fun f -> migrations_file := Some f), "<file> Generated migrations SQL for -migrate (read and appended in place)";
  ...
  | true, false ->
    ...
    let migrations_file =
      ...
      | Some f -> f
      | None -> fatal "-migrate requires -migrations-file <file>"
    in
    Migrate {
      gen_lang;
      name = !name;
...
let run_migrate ({ gen_lang; name; initial_files; target_files; migrations_file; extends_file; now; ddl_as_migration } : migrate_args) =
  ...
  let recorded_blocks () = merge_blocks (read_blocks migrations_file) ext in
