...
let parse_one' (sql, props) =
    ...
    let r = Syntax.parse sql in
    check_dialect r.sql r.dialect_features;
    begin match r.kind, !Gen.params_mode with
    | Insert (Some _, _), None -> Error.log "Cannot use `-params none` with autogenerated parameters"
    | _ -> ()
    end;
    let props = Props.set props "sql" r.sql in
    let props = match r.from_pos with
      | Some pos -> Props.set props "from_pos" (string_of_int pos)
      | None -> props
    in
    { Gen.schema = r.schema; vars = r.vars; kind = r.kind; props }
    ...
    | Some (buffer, props) ->
      ...
      | ReusableAndExecutable-> 
        ...
        | Some select_full -> 
          ...
          (* Extract from_pos for dynamic_select *)
          let from_pos = 
            let select = fst select_full.Sql.select_complete.select in
            select.Sql.from_pos
          in
          let props = Props.set props "sql" buffer in
          let props = match from_pos with
            | Some pos -> Props.set props "from_pos" (string_of_int pos)
            | None -> props
          in
