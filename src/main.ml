...
let parse_one' (sql, props) =
    ...
    let (sql,schema,vars,kind,all_resolved_cte_refs) = Syntax.parse sql in
    ...
    { Gen.schema; vars; kind; props; all_resolved_cte_refs }
...
let parse_one (sql,props as x) =
  ...
  | Some _ -> Some { Gen.schema=[]; vars=[]; kind=Stmt.Other; props=Props.set props "sql" sql; all_resolved_cte_refs=[] }
  | None -> parse_one x
