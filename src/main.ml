...
let get_dynamic_select props = match Props.get props "dynamic_select" with
  | Some "true" -> true
  | _ -> false
...
let parse_one' (sql, props) =
    ...
    Syntax.Config.dynamic_select := get_dynamic_select props;
    let { Syntax.sql; schema; vars; kind; dialect_features } = Syntax.parse sql in
