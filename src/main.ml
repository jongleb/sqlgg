...
let parse_one' (sql, elements, props) =
    if Sqlgg_config.debug1 () then Printf.eprintf "------\n%s\n%!" sql;
    let (sql, schema, vars, kind) = Syntax.parse elements in
...
let parse_one (sql, elements, props) =
  ...
  | None -> 
    try
      Some (parse_one' (sql, elements, props))
    with
...
let parse_select_one (sql, props) =
  try
    ...
    | Select select_full -> 
        Shared_queries.add (Option.default "" (Props.get props "name")) (((Stmt_elements.elements_to_sql sql)), select_full); 
        Some select_full
    | _ -> 
  ...
  with
  | Parser_utils.Error (exn, (line, cnum, tok, tail)) ->
      handle_parsing_error (Stmt_elements.elements_to_sql sql) exn (line, cnum, tok, tail)
  | exn ->
      preserve_exception_context ((Stmt_elements.elements_to_sql sql)) props exn
...
let get_statements ch =
  ...
  let extract_statement tokens =
    let elements = Stmt_elements.create () in
    let props = ref Props.empty in
    let answer () = elements, !props in
    ...
    let rec loop () =
      ...
      | Some x ->
        ...
        | `Semicolon ->
          Some (answer ())
        | `InStatementProp (n, v) ->
          Stmt_elements.add_prop (n, v) elements;
          loop()
    in
  ...
  let rec next () =
    ...
    | Some (buffer, props) ->
      ...
      in
      let sql = Stmt_elements.elements_to_sql buffer in
      match include_ with
      | OnlyReusable ->
      ...
      | OnlyExecutable -> 
        ...
        begin match parse_one (sql, buffer, props) with
        | None -> next ()
        | Some stmt -> 
          let stmt = { stmt with props = Props.update props "sql" sql; } in
          prerr_endline @@ Gen.show_stmt stmt;
          sql |> get_statement_error stmt |> Option.map_default (fun _ -> next ()) stmt
        end
      | ReusableAndExecutable-> 
        ...
        | Some select_full -> 
          ...
          let stmt = { Gen.schema; vars; kind; props = Props.set props "sql" sql; } in
          sql |> get_statement_error stmt |> Option.map_default (fun _ -> next ()) stmt
  in
