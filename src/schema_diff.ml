...
let diff_charset ~from_ ~to_ =
  if from_.Tables.tbl_charset = to_.Tables.tbl_charset then []
  else
    to_.Tables.tbl_charset |> Option.map_default
...
let diff_table ~from_ ~to_ =
  ...
  @ diff_indexes ~from_ ~to_ @ diff_charset ~from_ ~to_
...
let canonical ts =
  ...
  in
  let charset_sig = function
    | None -> ""
    | Some ({ charset; collation } : Tables.table_charset) ->
