...
module Attr_refinement = struct
  ...
  let inter a b = {
    ...
    meta = Qualified_attr.Map.merge (fun _ x y ->
      match x, y with Some x, Some y -> Meta.declared (Meta.inter x y) | _ -> None) a.meta b.meta }

  let keep_all = List.fold_left add empty
  let keep_shared = function [] -> empty | x :: l -> List.fold_left inter x l
...
let narrow_columns ~resolve ~constrains e =
  ...
  let rec narrow (e : Sql.expr) known =
    ...
    | `Has_value ->
      ...
      | Fun { kind; parameters; _ } -> keep_all (List.map same (Sql.strict_args kind parameters))
      | Case c -> every_path c
    ...
    | (`Holds | `Fails) as known ->
      let required =
        ...
        | Fun { kind = Quantified_comparison
                  { op = Comp_equal | Comp_num_cmp | Comp_num_eq; quantifier = `Any };
                parameters = e :: _; _ } ->
