...
| DynamicSelectJoin of { pid : param_id; pos : Pos.t; source : join_source }
...
let with_pos pos e =
  match e with
  | Fun f when Pos.equal f.fn_pos dummy_pos -> Fun { f with fn_pos = pos }
  | e -> e
...
type alter_action = [
    ...
    | `TtlOptions of ttl_option list * Pos.t
    | `RemoveTtl of Pos.t
    | `Cache of Pos.t
    | `NoCache of Pos.t
