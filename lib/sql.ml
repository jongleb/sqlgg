...
let comparison_signature op =
  ...
  | Comp_equal | Comp_num_cmp | Comp_num_eq -> Typ (depends Bool), [Var 0; Var 0]
...
let strict_args (kind : Source_type.t func) parameters =
  match kind, parameters with
  | (Comparison (Comp_equal | Comp_num_cmp | Comp_num_eq)
