...
module Qualified_attr = struct
  ...
  let of_attr (a : table_name Schema.Source.Attr.t) : t = { sources = a.sources; name = a.attr.name }
...
module Attr_refinement = struct
  ...
  let apply t (a : table_name Schema.Source.Attr.t) =
...
let propagate_meta ~meta_of =
  ...
  let carried = function [] -> Some (Meta.empty ()) | l -> Meta.common_all l in
  let class_meta same_domain = same_domain |> List.map fst |> carried in
  let context ~from_ret same_domain = class_meta same_domain |> Meta.common from_ret |> Meta.of_option in
  ...
  let rec aux e =
    ...
    | Of_values _ -> carried [], const e
    | InTupleList _ -> carried [], (fun _ -> map_sub_exprs without_meta e)
    ...
    | Fun ({ kind; parameters; _ } as fn) ->
      ...
      | None -> carried [], const (Fun { fn with kind; parameters = List.map without_meta parameters })
      | Some (ret, sig_args) ->
        let args = List.map aux parameters in
        let rebuild contexts = Fun { fn with kind; parameters = List.map2 push_to contexts args } in
        let is_var v = function Type.Var w -> Int.equal v w | Type.Typ _ -> false in
        let paired = List.combine sig_args args in
        let sharing v = List.filter_map (fun (arg, a) -> if is_var v arg then Some a else None) paired in
        let own = match ret with
          | Type.Var v -> class_meta (sharing v)
          | Type.Typ _ -> carried []
        in
        let push ctx =
          rebuild (sig_args |> List.map (function
            | Type.Var v ->
              context ~from_ret:(if is_var v ret then Meta.declared ctx else None) (sharing v)
            | Type.Typ _ -> Meta.empty ()))
        in
        own, push
  and unary x rebuild =
    let a = aux x in
...
and assign_types env expr =
  ...
  let rec typeof_ (e:res_expr) = (* FIXME simplify *)
    ...
    | ResFun { kind; parameters; is_over_clause}  ->
        ...
        | Agg (Pick | Compute), [_] -> let args, ret = convert Sql.agg_same_type in consider_agg_nullability ret, args
...
and meta_of ~env = function
  ...
  | `Select { select_complete = { select = ({columns = [{ value = Expr ({ value; _ }, _); _ }]; from; _}, _); _ }; cte } ->
    let (env,_,_) =
      eval_nested { env with ctes = fst (Option.map_default eval_cte ([], []) cte) @ env.ctes; scope = Subquery } from in
    Some (Meta.of_option (carried_meta ~env value))
  | `Select _ -> Some (Meta.empty ())

and carried_meta ~env e = fst (propagate_meta ~meta_of:(meta_of ~env) e)

and resolve_column_assignments ~env l =
  let open Schema.Source in 
  let open Attr in
  let all = all_tbl_columns (List.map (fun (a, b) -> a, Schema.Source.of_schema ~sources:[a] b) env.tables) in
  ...
  l |> List.map begin fun (col,expr) ->
    ...
    let attr =
      if env.is_update && Meta.get_is_non_nullifiable attr.attr.meta then
        Attr.map_attr (fun a ->
          { a with domain = Type.make_strict a.domain; extra = Constraints.add NotNull a.extra }) attr
      else attr in
    (* autoincrement is special - nullable on insert, strict otherwise *)
    let typ = Sql.make_collated ~collated:(
      if Constraints.mem Autoincrement attr.attr.extra
      then Type.nullable attr.attr.domain.t else attr.attr.domain) () in
    ...
    let with_default e = if not @@ Constraints.mem WithDefault attr.attr.extra then fail "Column %s doesn't have default value" col.cname else e in
    let assign e =
      let e = push_meta ~meta_of:(meta_of ~env) attr.attr.meta e in
    ...
    | { value = Expr ({ value = expr; _ }, alias); _ } ->
      ...
      let resolve_expr = function
        ...
        | e ->
          ...
          { Schema.Source.Attr.attr = unnamed_attribute ~meta:(Meta.of_option (carried_meta ~env e)) domain;
            sources = [] }
      in
      let col =
        match expr with
...
and do_join (env,params) { From.src; kind; cond; _ } =
  ...
  let common_columns = match cond with
    ...
    | On _ | Default -> []
  in
  let env = { env with schema = Schema.Join.join kind cond env.schema src.rsrc_schema;
              attr_refinement = Attr_refinement.keep_all (env.attr_refinement ::
                List.map (fun (col, referenced) -> Attr_refinement.inherit_meta ~constrains col ~referenced) common_columns) } in
  match cond with
...
let with_constraints attrs (constraints : Sql.table_constraints list) : Schema.t =
