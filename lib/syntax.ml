...
module Var_unifier : sig
  ...
  type node = { name : string; mutable parent : node option; mutable typ : Type.t option }
  ...
  let node t name =
    ...
    | Some n -> n
    | None -> let n = { name; parent = None; typ = None } in Hashtbl.add t name n; n

  let rec root n =
    n.parent |> Option.map_default (fun p -> let r = root p in if r != p then n.parent <- Some r; r) n
  ...
  let note t name typ =
    let r = root (node t name) in
    r.typ <- Some (Option.map_default (merge name typ) typ r.typ)

  let alias t n1 n2 =
    let r1 = root (node t n1) and r2 = root (node t n2) in
    if r1 != r2 then begin
      let typ =
        match r1.typ, r2.typ with
        | typ, None | None, typ -> typ
        | Some t1, Some t2 -> Some (merge r1.name t2 t1)
      in
      r2.parent <- Some r1;
      r2.typ <- None;
      r1.typ <- typ
  ...
  let typ t name ~default =
    match Hashtbl.find_opt t name with
    | None -> default
    | Some n -> Option.default default (root n).typ
end
...
let unify_params l =
  ...
  let register p signature =
    ...
    | Some `Dynamic, `Dynamic -> failed ~at:p.pos "dynamic select parameter %s is used several times (sharing dynamic selects is not implemented)" n
