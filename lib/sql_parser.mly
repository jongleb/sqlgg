...
column_def: name=IDENT kind=sql_type extra=column_def_extra*
  {
    ...
    let extra = Constraints.of_list @@ List.filter_map (Option.map (function
      ...
      | Parser_state.Other_extra c -> c
    )) extra in
    make_attribute name (Some kind) extra ~meta
  }
...
column_def_extra: PRIMARY? KEY { Some (Parser_state.Other_extra PrimaryKey) }
                | NOT NULL { Some (Parser_state.Other_extra NotNull) }
                | NULL { Some (Parser_state.Other_extra Null) }
                | UNIQUE KEY? { Some (Parser_state.Other_extra Unique) }
                | AUTOINCREMENT { Dialect_feature.set_autoincrement ($startofs, $endofs); Some (Parser_state.Other_extra Autoincrement) }
