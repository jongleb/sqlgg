...
The literal may come first, the annotated branch still decides the metadata:
  $ sqlgg -gen caml -no-header -dialect=mysql - <<'EOF' 2>&1
  ...
  > );
  > -- @literal_first
  > SELECT 'published' AS row_status UNION ALL SELECT status AS row_status FROM rows_with_status;
  > EOF
  module Sqlgg (T : Sqlgg_traits.M) = struct
  
    module IO = Sqlgg_io.Blocking
  
    let create_rows_with_status db  =
      T.execute db ("CREATE TABLE rows_with_status (\n\
      status ENUM('draft','published','failed') NOT NULL\n\
  )") T.no_params
  
    let literal_first db  callback =
      let invoke_callback stmt =
        callback
          ~row_status:(Row_status.get_column (T.get_column_string stmt 0))
      in
      T.select db ("SELECT 'published' AS row_status UNION ALL SELECT status AS row_status FROM rows_with_status") T.no_params invoke_callback
  
    module Fold = struct
      let literal_first db  callback acc =
        let invoke_callback stmt =
          callback
            ~row_status:(Row_status.get_column (T.get_column_string stmt 0))
        in
        let r_acc = ref acc in
        IO.(>>=) (T.select db ("SELECT 'published' AS row_status UNION ALL SELECT status AS row_status FROM rows_with_status") T.no_params (fun x -> r_acc := invoke_callback x !r_acc))
        (fun () -> IO.return !r_acc)
