...
-- transitive chain: avatars' ON references profiles, so a column from avatars pulls
-- BOTH joins in; a column from profiles pulls only profiles
...
-- three-level chain accumulates the whole ancestor closure, child first
-- [sqlgg] dynamic_select=true
-- @chain3
...
-- diamond: two children of one parent share the parent in both closures
-- [sqlgg] dynamic_select=true
-- @diamond
