module Sqlgg (T : Sqlgg_traits.M) = struct
  ...
  module Nonuniq_col = struct
    ...
    let select db (col : _ t) ~uid callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u LEFT JOIN orders o ON o.user_id = u.id WHERE u.id = ?")
      set_params (fun row -> let (__sqlgg_r_col, __sqlgg_idx_after_col) = col.projection.read row 0 in callback
          __sqlgg_r_col)
  ...
  module Ref_in_where_col = struct
    ...
    let select db (col : _ t) ~b callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u LEFT JOIN profiles p ON p.user_id = u.id WHERE p.bio = ?")
      set_params (fun row -> let (__sqlgg_r_col, __sqlgg_idx_after_col) = col.projection.read row 0 in callback
          __sqlgg_r_col)
