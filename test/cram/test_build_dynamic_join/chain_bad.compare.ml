module Sqlgg (T : Sqlgg_traits.M) = struct
  ...
  module Chain_bad_col = struct
    ...
    let select db (col : _ t) ~uid callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u LEFT JOIN profiles p ON p.user_id = u.id LEFT JOIN avatars a ON a.profile_id = p.id WHERE u.id = ?")
