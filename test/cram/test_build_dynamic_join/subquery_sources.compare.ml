module Sqlgg (T : Sqlgg_traits.M) = struct
  ...
  module Join_subq_source_col = struct
    type source = |
    ...
    let select db (col : _ t) ~uid callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u LEFT JOIN (SELECT user_id, bio FROM profiles) s ON s.user_id = u.id WHERE u.id = ?")
  ...
  module Subq_join_dup_col = struct
    type source = |
    ...
    let select db (col : _ t) ~uid callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u LEFT JOIN (SELECT p.user_id, p.bio FROM profiles p, users x) s ON s.user_id = u.id WHERE u.id = ?")
      set_params (fun row -> let (__sqlgg_r_col, __sqlgg_idx_after_col) = col.projection.read row 0 in callback
          __sqlgg_r_col)
  ...
  module Subq_union_dup_col = struct
    type source = |
    ...
    let select db (col : _ t) ~uid callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u LEFT JOIN (SELECT user_id, bio FROM profiles UNION ALL SELECT user_id, bio FROM profiles) s ON s.user_id = u.id WHERE u.id = ?")
      set_params (fun row -> let (__sqlgg_r_col, __sqlgg_idx_after_col) = col.projection.read row 0 in callback
          __sqlgg_r_col)
  ...
  module Subq_base_join_col = struct
    ...
    let select db (col : _ t) callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM (SELECT id FROM users) s " ^ (if List.mem Profiles col.deps then " " ^ "LEFT JOIN profiles p ON p.user_id = s.id" else "") ^ "")
      set_params (fun row -> let (__sqlgg_r_col, __sqlgg_idx_after_col) = col.projection.read row 0 in callback
          __sqlgg_r_col)
