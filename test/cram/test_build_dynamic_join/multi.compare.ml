module Sqlgg (T : Sqlgg_traits.M) = struct
  ...
  module Two_indep_col = struct
    ...
    let select db (col : _ t) ~uid callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u " ^ (if List.mem Profiles col.deps then " " ^ "LEFT JOIN profiles p ON p.user_id = u.id" else "") ^ " " ^ (if List.mem Avatars col.deps then " " ^ "LEFT JOIN avatars a ON a.id = u.id" else "") ^ " WHERE u.id = ?")
  ...
  module Subq_in_on_col = struct
    ...
    let select db (col : _ t) ~uid callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u LEFT JOIN profiles p ON p.user_id = u.id LEFT JOIN avatars a ON a.id = (SELECT MAX(id) FROM avatars) WHERE u.id = ?")
      set_params (fun row -> let (__sqlgg_r_col, __sqlgg_idx_after_col) = col.projection.read row 0 in callback
          __sqlgg_r_col)
