module Sqlgg (T : Sqlgg_traits.M) = struct
  ...
  module Param_in_on_col = struct
    ...
    let select db (col : _ t) ~b ~uid callback =
      let set_params stmt =
        let p = T.start_params stmt (2 + col.projection.count) in
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u LEFT JOIN profiles p ON p.user_id = u.id AND p.bio = ? WHERE u.id = ?")
