...
-- a LEFT JOIN of a subquery is never droppable: its columns inherit the underlying
-- table's UNIQUE/PRIMARY marks, but the subquery may multiply rows -> keep
-- [sqlgg] dynamic_select=true
-- @join_subq_source
SELECT u.id, s.bio FROM users u LEFT JOIN (SELECT user_id, bio FROM profiles) s ON s.user_id = u.id WHERE u.id = @uid;
-- cross join inside: user_id is genuinely NOT unique anymore -> keep
-- [sqlgg] dynamic_select=true
-- @subq_join_dup
...
-- UNION ALL inside: duplicates -> keep
-- [sqlgg] dynamic_select=true
-- @subq_union_dup
...
-- a subquery as the BASE source does not poison a droppable table join on top of it
-- [sqlgg] dynamic_select=true
-- @subq_base_join
