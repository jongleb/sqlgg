module Sqlgg (T : Sqlgg_traits.M) = struct
  ...
  module Chain_pinned_col = struct
    ...
    let select db (col : _ t) ~label callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u LEFT JOIN profiles p ON p.user_id = u.id LEFT JOIN avatars a ON a.id = p.avatar_id LEFT JOIN badges b ON b.id = a.badge_id WHERE b.label = ?")
      set_params (fun row -> let (__sqlgg_r_col, __sqlgg_idx_after_col) = col.projection.read row 0 in callback
          __sqlgg_r_col)
