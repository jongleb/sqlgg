  ...
  module Sqlgg (T : Sqlgg_traits.M) = struct
    ...
    module Products_dyn_col = struct
      ...
      let select db (col : _ t) ~min callback =
        ...
        in
        T.select db
        ("SELECT * FROM (SELECT " ^ col.projection.column ^ " FROM products WHERE price > ?) AS sub")
    ...
    module Cols_over_subq_col = struct
      ...
      let select db (col : _ t) ~id callback =
        ...
        in
        T.select db
        ("SELECT " ^ col.projection.column ^ " FROM (SELECT id, name, price FROM products) AS sub WHERE sub.id = ?")
        set_params (fun row -> let (__sqlgg_r_col, __sqlgg_idx_after_col) = col.projection.read row 0 in callback
            __sqlgg_r_col)
