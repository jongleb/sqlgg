module Sqlgg (T : Sqlgg_traits.M) = struct
  ...
  let create_accounts db  =
    T.execute_unprepared db ("CREATE TABLE accounts (\n\
  id BIGINT NOT NULL,\n\
    cid BIGINT NOT NULL,\n\
  ...
  let create_events db  =
    T.execute_unprepared db ("CREATE TABLE events (\n\
  order_ref BIGINT NOT NULL,\n\
  amount BIGINT NOT NULL\n\
)")

  let create_named_owners db  =
    T.execute_unprepared db ("CREATE TABLE named_owners (\n\
    id BIGINT NOT NULL,\n\
  title TEXT NOT NULL\n\
)")

  let create_named_owned db  =
    T.execute_unprepared db ("CREATE TABLE named_owned (\n\
  id BIGINT NOT NULL,\n\
  note TEXT NOT NULL\n\
)")

  let create_other_domain db  =
    T.execute_unprepared db ("CREATE TABLE other_domain (\n\
    id BIGINT NOT NULL\n\
)")

  let create_owners db  =
    T.execute_unprepared db ("CREATE TABLE owners (\n\
    id BIGINT NOT NULL PRIMARY KEY\n\
)")

  let create_owned db  =
    T.execute_unprepared db ("CREATE TABLE owned (\n\
  ...
  let create_unrelated db  =
    T.execute_unprepared db ("CREATE TABLE unrelated (\n\
  owner_ref BIGINT NOT NULL\n\
)")

  let create_projects db  =
    T.execute_unprepared db ("CREATE TABLE projects (\n\
  id BIGINT NOT NULL,\n\
    company_id BIGINT NOT NULL\n\
)")

  let create_alerts db  =
    T.execute_unprepared db ("CREATE TABLE alerts (\n\
  dashboard_id BIGINT NOT NULL,\n\
  company_id BIGINT NOT NULL\n\
)")

  let create_courses db  =
    T.execute_unprepared db ("CREATE TABLE courses (\n\
    slug TEXT NOT NULL,\n\
    seconds BIGINT NOT NULL\n\
)")

  let create_left_rows db  =
    T.execute_unprepared db ("CREATE TABLE left_rows (\n\
    id BIGINT NOT NULL\n\
)")

  let create_right_rows db  =
    T.execute_unprepared db ("CREATE TABLE right_rows (\n\
    id BIGINT NOT NULL,\n\
    status ENUM('draft','published','failed') NOT NULL\n\
)")

  let create_codec_rows db  =
    T.execute_unprepared db ("CREATE TABLE codec_rows (\n\
  id INT NOT NULL,\n\
