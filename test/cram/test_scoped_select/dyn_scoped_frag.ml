module Frag (T : Sqlgg_traits.M with
  ...
  let _q1 db = Dscope_q1_col.select db (who_of_scope (module Dscope_q1_col)) ~id:1L
  let _q2 db =
    Dscope_q2_col.select db (who_of_scope (module Dscope_q2_col)) ~min_stock:10L (fun _ -> ())
end
