...
type tinted = { id : int64; color : Color.t } [@@deriving sqlgg { mode = dynamic }]
type named = { name : string option } [@@deriving sqlgg { mode = dynamic }]
...
let () =
  ...
  let frag =
    Tinted_q1_col.(
      let+ t = tinted_of_dyn (module Cols)
      and+ n = named_of_dyn (module Cols) in
      (t, n))
  in
  begin match Tinted_q1_col.select () frag ~id:7L with
  | Some (t, n) ->
    show_tinted "Q1" t;
  ...
  Tinted_q2_col.(select () (tinted_of_dyn (module Cols)) ~min_id:1L)
    (show_tinted "Q2")
