...
> **Note:** This doesn't have to be an `executable` — you can use `library` or any other stanza as needed.
...
The `(T : Sqlgg_traits.M)` functor parameter and `module IO = Sqlgg_io.Blocking` will be covered in detail in the [Traits](traits.md) chapter (OCaml-specific) — for now, we can skip over these details.
