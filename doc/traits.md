...
[← Back to Main](./index.md) | [← Previous: DML/DQL](./dml-dql.md)

---

> **Note:** Traits are supported in **OCaml** and **C++**. Other target languages currently have hardcoded type mappings.
...
> For a blocking example, see [Getting Started](./getting-started.md#running-the-example). This section focuses on the async (Lwt) setup.
...
Note: without `Db.List.` or `Db.Fold.` the callback returns `unit` and rows are processed directly — no need to build a list first and then iterate.
