...
let analysis stmt =
  ...
  | Verbatim -> { scope = { sources = []; ctes = [] }; select_scopes = []; exprs = [] }
  | Rejected (_, analysis) | Checked (_, analysis) -> analysis
...
let errors stmt =
  ...
  @ match stmt.outcome with
    | Verbatim -> []
    | Rejected (err, _) -> [ err ]
    | Checked (checked, _) -> checked.dialect_errors

let tokens stmt = Lazy.force stmt.tokens

let params stmt =
  match stmt.outcome with
  | Checked (checked, _) -> checked.params
  | Verbatim | Rejected _ -> []

let exprs stmt = (analysis stmt).exprs

let select_scope stmt offset =
  let scopes = List.to_seq (analysis stmt).select_scopes in
...
let enclosing_scope stmt offset =
  Option.value (select_scope stmt offset) ~default:(analysis stmt).scope

let statement_scope stmt = symbols (analysis stmt).scope
let scope stmt offset = symbols (enclosing_scope stmt offset)
let sources stmt offset = (enclosing_scope stmt offset).sources
...
let check ~file (stmt : Statements.t) =
  ...
  let outcome =
    match compiled with
    | `Verbatim -> Verbatim
    | `Rejected err -> Rejected (err, analysis)
    | `Checked c -> Checked (c, analysis)
  in
  { pos = stmt.pos; name = Props.name stmt.props; props_errors;
    tokens = lazy (Recover_parser.tokens stmt.text); outcome }
...
let analyze ?(cache = Cache.create ()) ~path text =
  let apply ~file index blocks =
    List.fold_left_map (fun index block ->
      ...
      match stmt.outcome with
      | Checked ({ new_table = Some table; _ }, _) ->
        Symbol.Index.add table.name table index, statement
      | Checked ({ new_table = None; _ }, _) | Verbatim | Rejected _ ->
