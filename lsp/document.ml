...
let select_scope_at_opt stmt offset =
  Sql.Pos.find_innermost_covering_opt offset (List.to_seq stmt.select_scopes)
  |> Option.map fst

let scope_at stmt offset =
  select_scope_at_opt stmt offset |> Option.value ~default:stmt.scope
...
let find_reusable_opt t name =
  List.find_map (fun (statement : checked_statement) ->
...
let check ~file (stmt : Statements.t) =
  ...
  let rebase (start, stop) =
    Sql.Pos.shift base
      (Sql.Pos.clamp_offset stmt.text start,
       Sql.Pos.clamp_offset stmt.text stop)
  in
  ...
  let make_scope src_tbls (table_aliases : Sql.table_alias list) cte_tables =
    ...
    let symbol (table, schema) =
      ...
      let definition =
        ...
        | None ->
          Option.bind (Sql.find_table_alias aliases name)
            (fun table -> find_cte_opt (tn table))
      in
      let columns =
      ...
      in
      let kind =
        if Option.is_some cte then Symbol.Cte else Symbol.Local
      in
...
let analyze ?(cache = Cache.create ()) ~path text =
  let apply ~file index blocks =
    List.fold_left_map (fun index block ->
      ...
      | Ok { new_table = None; _ } | Skip | Error _ ->
  ...
  | Some _ | None ->
    ...
    let result = { path; statements; index; snapshot = schema.snapshot } in
